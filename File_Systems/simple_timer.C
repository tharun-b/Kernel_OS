...
#ifdef SCHE_ROUND_ROBIN
	interrupt_rr_qt++;
/* incremented counter for round robin case */
	if(interrupt_rr_qt >= (hz/20) )/* this comparison makes sure that every 50ms interrupt is handled */
	{
		interrupt_rr_qt =0;
		if(SYSTEM_SCHEDULER->queue_length) /* To Avoid switching when queue lentgh is 0 *//* befor starting threads if this is called it will be neglected */
		{
			SYSTEM_SCHEDULER->resume(Thread::CurrentThread());
			send_EOI_to_IPC(_r); /* Notify IPC about the registry and interrupt handling moment */
			//This can be wrapped in a function and handled in Interrput Handler
			//Notifying the PIC that interrupt has been handled
			SYSTEM_SCHEDULER->yield();
		}
#if 0
		Console::puts("50 ms Passed --> Switch threads for RR\n");
#endif
	}
#endif
