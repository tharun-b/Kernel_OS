...
/* The scheduler may need private members... */
   ...
   void Scheduler::yield()
   {
	   ...
	   {
#if DEBUG_ENABLE
	/*	   Console::puts("status of queue T@\t");
		   Console::putui((unsigned int)(thread_ready_queue[head_of_queue]));
		   Console::putui((unsigned int)(thread_ready_queue[head_of_queue+1]));
		   Console::putui((unsigned int)(thread_ready_queue[head_of_queue+2]));
		   Console::putui((unsigned int)(thread_ready_queue[head_of_queue+3]));*/
#endif
...
#endif
		  if(thread_ready_queue[rnext_thread])
			   Thread::dispatch_to(thread_ready_queue[rnext_thread]);

#if DEBUG_ENABLE
		   else
		   {
			   Console::puts("Asked to switch to empty thread\n");
		   }
#endif
	  ...
	  // Console::puts("Enabling interrupts ack\n");
