...
//    abort();
  ...
  /* Made a separate functon for this */
...
/* this is a notify IPC function which is created just to say that
 * A round robin case where end of quantum interrupt is dispatching to other
 * thred, as interrupt is handled - The thred calls this function to take care of
 * the hardware flags
 */
/* This snippet of code
 *  can be observed in the last part of the above interrupt hadnler routing */

void InterruptHandler::send_EOI_to_IPC(REGS *_r)
{
	{
		unsigned int int_no = _r->int_no - IRQ_BASE;

		if (this->generated_by_slave_PIC(int_no))
		{
			outportb(0xA0, 0x20);
		}
		// Send an EOI message to the master interrupt controller.
