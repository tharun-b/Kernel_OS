...
unsigned long FramePool::get_frame() {
	...
	unsigned int frame_sub_index = 0;
			...
			//Console::putui();
			Console::puti(bit_map_ptr[cur_char_idx]);
#endif
	...
	}
	{
		...
		while ((char_with_slot & pos_eval) == 0) {
			frame_sub_index++;
			pos_eval = pos_eval << 2;
			if(frame_sub_index > 3){
				Console::puts("ERROR IN INDEX CALCUAL");
				break;
			}
		}
		...
		Console::putui(cur_char_idx);
		Console::putui(frame_sub_index);
...
void FramePool::mark_inaccessible(unsigned long _base_frame_no,
	...
	if ((_base_frame_no < base_frame_num)
			|| (_base_frame_no > base_frame_num + num_frames)
			|| (_base_frame_no + _nframes > base_frame_num + num_frames))
	{
		Console::puts("Illegal Access of the frames\n");
	}
...
void FramePool::release_frame(unsigned long _frame_no) {
	...
	if(info_frame_num == _frame_no)
	{
		Console::puts(
						"Cant release the info frame \n");
				return;
	}
